...
export default {
	...
	methods:{
		...
		deleteReview: function(modCode, user) {
			...
			db.collection('reviews').doc(modCode).set(
				{ module_reviews : {[user]: db.FieldValue.delete()}}, { merge: true });
			db.collection('reviews').doc(modCode).get().then( doc => {
				...
				if(Object.keys(module_review).length == 0) {
					console.log("deleted");
					db.collection('reviews').doc(modCode).delete().then(() => {
						this.updateReviews();
					});
				}
				module_review = {};
